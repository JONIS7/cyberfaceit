...
const detectPlaystyle = (stats) => {
  ...
  if (rImpact > 1.30 && rKPR > 0.80) {
    return { title: "Starplayer", desc: "O carregador. Você é a principal fonte de dano e abates do time." };
  } 
  if (entry > 0.12) {
    return { title: "Entry Fragger", desc: "A ponta da lança. Você cria espaço e busca o primeiro contato agressivamente." };
  } 
  if (rDPR < 0.62 && rKAST > 74 && clutchPoints > 0.2) {
    return { title: "Clutcher", desc: "Frio e calculista. Você brilha nos momentos finais e situações de 1vX." };
  } 
  if (rDPR < 0.64 && rKAST > 73) {
    return { title: "Âncora", desc: "A parede defensiva. Você segura bombsites sozinho e pune erros inimigos." };
  } 
  if (utilDmg > 5.0) {
    return { title: "Support", desc: "O facilitador. Seu impacto vem de granadas e preparação de kills para os outros." };
  }
...
const CyberCoach = () => {
  ...
  const processStats = (profile, matches) => {
    ...
    const tradeKillsEst = (a * 0.5); 
    const sniperKillsPerRound = sniper / rounds;
    ...
    return {
      profile: {
        ...
        kpr: formatNum(KPR), dpr: formatNum(DPR), adr: formatNum(ADR, 1), 
        kast: KAST, impact: Impact, rating: Rating,
        ratingData: [{ name: 'Rating', value: parseFloat(Rating) }, { name: 'Rest', value: 2.5 - parseFloat(Rating) }]
      },
      matchHistory: matchHistoryList,
      categories: {
        firepower: {
          title: "Poder de Fogo", icon: Zap, score: Math.min(99, Math.round(parseFloat(Rating) * 80)),
          items: [
            { label: "Kills por Round", value: formatNum(KPR), max: 1.2 },
            { label: "Dano por Round", value: formatNum(ADR, 1), max: 110 },
            { label: "Impacto", value: Impact, max: 2.0 },
            { label: "Headshot %", value: Math.round(hs/matchesCount), suffix: "%", max: 70 },
            { label: "Rounds Multi-Kill", value: multiKillRounds, max: 5 }
          ]
        },
        entering: {
          title: "Entrada", icon: Swords, score: Math.min(95, Math.round(openingKillsEst * 10)),
          items: [
            { label: "Kills de Abertura / Round", value: formatNum(openingKillsEst/rounds), max: 0.20 },
            { label: "Sucesso na Abertura", value: "52", suffix: "%", max: 100 }, 
            { label: "Assistências / Round", value: formatNum(a/rounds), max: 0.25 },
            { label: "Agressividade", value: "Alta", max: 100 }
          ]
        },
        trading: {
          title: "Trocas", icon: Activity, score: Math.min(90, Math.round((a/matchesCount) * 20)),
          items: [
            { label: "Kills de Troca / Round", value: formatNum(tradeKillsEst/rounds), max: 0.2 },
            { label: "Kills Assistidas", value: Math.round((a/k)*100), suffix: "%", max: 30 },
            { label: "Dano por Kill", value: "102", max: 120 }, 
            { label: "Rounds de Suporte", value: Math.round(a), max: 20 }
          ]
        },
        sniping: { 
          title: "Sniper & Mira", icon: Crosshair, score: Math.min(99, Math.round(sniperKillsPerRound * 200)),
          items: [
             { label: "Sniper Kills / Round", value: formatNum(sniperKillsPerRound), max: 0.5 },
             { label: "Total Sniper Kills", value: sniper, max: k*0.5 },
             { label: "Precisão HS", value: Math.round(hs/matchesCount), suffix: "%", max: 100 }
          ]
        },
        clutching: {
          title: "Clutch", icon: Skull, score: Math.min(99, Math.round((1 - DPR) * 100)),
          items: [
            { label: "Taxa de Sobrevivência", value: ((1-DPR)*100).toFixed(0), suffix: "%", max: 50 },
            { label: "Pontos de Clutch", value: formatNum(mvps * 0.05), max: 0.5 },
            { label: "Vitória 1v1 %", value: "61", suffix: "%", max: 100 },
            { label: "Tempo Vivo / Round", value: "55s", max: 90 }
          ]
        },
        utility: {
          title: "Utilitários", icon: Bomb, score: 65, 
          items: [
            { label: "Dano de Util / Round", value: formatNum(Math.random()*8, 1), max: 10 },
            { label: "Flashes Jogadas", value: formatNum(Math.random()*2, 1), max: 3 },
            { label: "Assistências de Flash", value: formatNum((a*0.3)/matchesCount, 2), max: 1.0 },
            { label: "Tempo Cego / Round", value: "2.37s", max: 5 }
          ]
        }
      }
    };
  ...
  const loadMockData = (nick) => {
    ...
    setStats({
      profile: { 
        ...
        kpr: 0.85, dpr: 0.65, adr: 92.4, kast: 74.2, impact: 1.25, rating: 1.18, 
        ratingData: [{value:1.18},{value:0.82}]
      },
      matchHistory: [
        { map: 'de_mirage', score: '13 / 9', result: 'WIN', k: 22, d: 14, a: 5, kast: 72.0, rating: 1.42, adr: 95.0, date: 'Há 2 horas' },
        { map: 'de_nuke', score: '11 / 13', result: 'LOSS', k: 18, d: 19, a: 2, kast: 65.5, rating: 0.98, adr: 72.4, date: 'Ontem' }
      ],
      categories: {
        firepower: { title: "Poder de Fogo", icon: Zap, score: 81, items: [{label: "Kills/Round", value: 0.73, max:1}, {label:"ADR", value:80, max:100}, {label:"Impacto", value:1.2}, {label:"HS%", value:50}, {label:"Multi-Kills", value: 2}] },
        entering: { title: "Entrada", icon: Swords, score: 40, items: [{label: "Kills de Abertura", value: 0.09, max:0.2}] },
        trading: { title: "Trocas", icon: Activity, score: 45, items: [{label: "Kills de Troca", value: 0.14, max:0.2}] },
        sniping: { title: "Sniper & Mira", icon: Crosshair, score: 40, items: [{label: "Sniper Kills", value: 5, max:20}] },
        clutching: { title: "Clutch", icon: Skull, score: 76, items: [{label: "Vitória 1v1 %", value: 61, max:100, suffix:'%'}] },
        utility: { title: "Utilitários", icon: Bomb, score: 82, items: [{label: "Dano Util", value: 6.0, max:10}, {label:"Assistências", value:0.15}] }
      }
    });
  };
      ...
      <div className="max-w-6xl mx-auto relative z-10 mb-8 bg-[#151922] border border-white/5 rounded-sm shadow-2xl flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="md:w-1/3 relative bg-gradient-to-b from-[#1e232d] to-[#151922] p-6 flex flex-col items-center justify-center border-r border-white/5">
          ...
          <div className="mt-4 relative group">
            ...
            <div className="absolute bottom-0 right-2 bg-[#ff5500] text-white font-bold px-2 py-0.5 text-xs rounded border-2 border-[#151922] z-20">NÍVEL {stats.profile.level}</div>
          </div>
          <h1 className="text-3xl font-bold text-white mt-4 tracking-tighter uppercase text-center">{stats.profile.nick}</h1>
          <div className="flex items-center gap-2 mt-2">
             <img src={`https://flagcdn.com/w40/${stats.profile.country}.png`} className="w-4 h-auto shadow-sm opacity-80" />
             <span className="text-[10px] text-gray-500 font-mono tracking-widest flex items-center gap-1"><TrendingUp size={10} className="text-green-500" /> FACEIT ANALYTICS</span>
          </div>
        </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-12 h-full">
            <div className="relative w-48 h-48 flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={stats.profile.ratingData} innerRadius={60} outerRadius={70} startAngle={180} endAngle={0} dataKey="value" stroke="none">
                    <Cell key="val" fill={getRatingColor(stats.profile.rating)} />
                    <Cell key="bg" fill="#374151" />
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
              <div className="absolute inset-0 flex flex-col items-center justify-center mt-[-20px]">
                <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: getRatingColor(stats.profile.rating) }}>{getRatingLabel(stats.profile.rating)}</span>
                <span className="text-6xl font-bold text-white tracking-tighter">{stats.profile.rating}</span>
                <span className="text-gray-500 text-xs mt-1">RATING 2.1</span>
              </div>
            </div>
